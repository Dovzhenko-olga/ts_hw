...
if (confirm('Start?')) {
    ...
    function generateDaysArray(count, workDays, weekend, holydays) {
        const days = [];
    ...
    function checkDaysArray(days, weekend, holydays) {
        let weekendCount = 0;
        let holydaysCount = 0;
        let compareResult;
        days.forEach(day => {
            if (weekend.includes(day))
                weekendCount++;
            else if (holydays.includes(day))
                holydaysCount++;
        });
        if (weekendCount > holydaysCount)
            compareResult = 'Вихідних більше, ніж святкових днів.';
        else if (weekendCount < holydaysCount)
            compareResult = 'Вихідних менше, ніж святкових днів.';
        else
            compareResult = 'Вихідних та святкових днів була однакова кількість.';
